...
public class CacheConfiguration {
        ...
        log.info("Defining Infinispan Global Configuration");
            if (this.registration == null) { // if registry is not defined, use native discovery
                ...
                return () -> GlobalConfigurationBuilder
                    .defaultClusteredBuilder().defaultCacheName("infinispan-jhdemo-cluster-cache").transport().defaultTransport()
                    .addProperty("configurationFile", jHipsterProperties.getCache().getInfinispan().getConfigFile())
                    .clusterName("infinispan-jhdemo-cluster").globalJmxStatistics()
                    .enabled(jHipsterProperties.getCache().getInfinispan().isStatsEnabled())
                    .allowDuplicateDomains(true).build();
            }
            return () -> GlobalConfigurationBuilder
                    .defaultClusteredBuilder().defaultCacheName("infinispan-jhdemo-cluster-cache").transport()
    ...
    class InfinispanJCacheManager extends JCacheManager {
        ...
        public InfinispanJCacheManager(URI uri, EmbeddedCacheManager cacheManager, CachingProvider provider,
            ...
            registerPredefinedCache(com.example.repository.UserRepository.USERS_BY_LOGIN_CACHE, new JCache<Object, Object>(
                cacheManager.getCache(com.example.repository.UserRepository.USERS_BY_LOGIN_CACHE).getAdvancedCache(), this,
                ConfigurationAdapter.create()));
            registerPredefinedCache(com.example.repository.UserRepository.USERS_BY_EMAIL_CACHE, new JCache<Object, Object>(
                cacheManager.getCache(com.example.repository.UserRepository.USERS_BY_EMAIL_CACHE).getAdvancedCache(), this,
                ConfigurationAdapter.create()));
            registerPredefinedCache(com.example.domain.User.class.getName(), new JCache<Object, Object>(
                cacheManager.getCache(com.example.domain.User.class.getName()).getAdvancedCache(), this,
                ConfigurationAdapter.create()));
            registerPredefinedCache(com.example.domain.Authority.class.getName(), new JCache<Object, Object>(
                cacheManager.getCache(com.example.domain.Authority.class.getName()).getAdvancedCache(), this,
                ConfigurationAdapter.create()));
            registerPredefinedCache(com.example.domain.User.class.getName() + ".authorities", new JCache<Object, Object>(
                cacheManager.getCache(com.example.domain.User.class.getName() + ".authorities").getAdvancedCache(), this,
                ConfigurationAdapter.create()));
